...
contains
   ...
   subroutine CNEvergreenPhenologyLeaf(num_soilp, filter_soilp, &
      ...
      ! !DESCRIPTION:
      ! For coupled carbon-nitrogen code (CN).
      ! This routine handles the evergreen phenology code:
      ! explicit temperature-controlled phenology for  the evergreen needleleaf boreal tree
      ! no explicit phenology for other evergree.n types
      !
      !-----------------------------------------------------------------------
      ! !USES:
      use clm_varcon, only: secspday
      ...
      use clm_varcon, only: secspday
      use pftvarcon, only: ndllf_evr_brl_tree, grperc, grpnow
            ...
            if (ivt(p) == ndllf_evr_brl_tree) then
               ...
               if (dormant_flag(p) == 1.0_r8) then
                  ...
                  if (onset_gdd(p) > crit_onset_rf) then
                     ...
                     gresp_storage_to_xfer(p) = leafc_storage_to_xfer(p)*grperc(ivt(p))*(1._r8 - grpnow(ivt(p)))
                     if (woody(ivt(p)) == 1.0_r8) then
                        ...
                        ! remove the corresponding amount of growth respiration
                        gresp_storage_to_xfer(p) = gresp_storage_to_xfer(p) + &
                                                   (livestemc_storage_to_xfer(p) + deadstemc_storage_to_xfer(p))* &
                                                   grperc(ivt(p))*(1._r8 - grpnow(ivt(p)))
                     end if
                     ! prevent storage from going to zero
                     gresp_storage_to_xfer(p) = min(gresp_storage_to_xfer(p), gresp_storage(p)/dt)
   ...
   subroutine CNEvergreenPhenologyRoot(num_soilp, filter_soilp, temperature_vars, cnstate_vars, &
      ...
      ! !USES:
      use shr_const_mod, only: SHR_CONST_TKFRZ, SHR_CONST_PI
      use clm_varcon, only: secspday, denh2o
      use clm_time_manager, only: get_days_per_year, get_curr_date, timemgr_datediff
      use pftvarcon, only: ndllf_evr_brl_tree, grperc, grpnow
      ...
      associate ( &
         ...
         lfr_froot_td => cnstate_vars%lfr_froot_td_patch, & ! Output:  [real(r8) (:) ] () temperature scaling factor on the fine root litterfall rate
         lfr_froot_wd => cnstate_vars%lfr_froot_wd_patch & ! Output:  [real(r8) (:) ] () water scaling factor on the fine root litterfall rate
         )
         ...
         do fp = 1, num_soilp
            ...
            if (ivt(p) == ndllf_evr_brl_tree) then
               ...
               ! function of soil temperature and soil water potential
               t_soi_avg = 0._r8
               h2osoi_liq_avg = 0._r8
               psi_max = -9999._r8
               do j = 1, nlevsoi
                  t_soi_avg = t_soi_avg + rootfr(p, j)*t_soisno(c, j)
                  h2osoi_liq_avg = h2osoi_liq_avg + rootfr(p, j)*h2osoi_liq(c, j)/(col_pp%dz(c, j)*denh2o)
               ...
               ! -------------------------------------------------------------
               ! Calculate onset & offset flags, dynamic allocation factor
               ! -------------------------------------------------------------
               ! update offset_counter and test for the end of the declining period
               ...
               ! update onset_counter and test for the end of the onset period
               if (onset_flag_root(p) == 1.0_r8) then
                  ! decrement counter for onset period
                  onset_counter_root(p) = onset_counter_root(p) - dt*onset_froot_fnmin(p)*onset_froot_fw(p)

                  ! if this is the end of the onset period, or storage has been depleted,
                  ! reset phenology flags and indices
                  if (onset_counter_root(p) <= 0.0_r8) then
                     ! this code block was originally handled by call cn_onset_cleanup(p)
                     ! inlined during vectorization
                     onset_flag_root(p) = 0.0_r8
                     onset_counter_root(p) = 0.0_r8
                     ! set all transfer growth rates to 0.0
               ...
               ! test for switching from dormant period to growth period
               if (dormant_flag_root(p) == 1.0_r8) then
                  ! Test to turn on growing degree-day sum, if off.
                  ! switch on the growing degree day sum on the winter solstice
                  ...
                  if ((onset_gdd_root(p) > crit_onset_rf_root) .or. (onset_flag(p) == 1._r8)) then
                     ...
                     gresp_storage_to_xfer(p) = frootc_storage_to_xfer(p)*grperc(ivt(p))*(1._r8 - grpnow(ivt(p)))
                     if (woody(ivt(p)) == 1.0_r8) then
                        ...
                        gresp_storage_to_xfer(p) = gresp_storage_to_xfer(p) + &
                                                   (livecrootc_storage_to_xfer(p) + deadcrootc_storage_to_xfer(p))* &
                                                   grperc(ivt(p))*(1._r8 - grpnow(ivt(p)))
                     end if
                     ! prevent storage from going to zero
                     gresp_storage_to_xfer(p) = min(gresp_storage_to_xfer(p), gresp_storage(p)/dt)
                     ...
                     if (t_soi_avg .lt. hardiness_root(ivt(p))) then
                        ...
                        ! which is proportional to display, not storage, but I mustn't
                        ! make the storage go to zero either
                        frootc_storage_to_xfer(p) = min(frootc(p), frootc_storage(p))*bgtr(p)
                        gresp_storage_to_xfer(p) = frootc_storage_to_xfer(p)*grperc(ivt(p))*(1._r8 - grpnow(ivt(p)))
                        if (woody(ivt(p)) == 1.0_r8) then
                           livecrootc_storage_to_xfer(p) = min(livecrootc(p), frootc_storage(p))*bgtr(p)
                           deadcrootc_storage_to_xfer(p) = min(deadcrootc(p), deadcrootc_storage(p))*bgtr(p)
                        ...
                        ! set nitrogen fluxes for shifting storage pools to transfer pools
                        frootn_storage_to_xfer(p) = min(frootn(p), frootn_storage(p))*bgtr(p)
                        if (woody(ivt(p)) == 1.0_r8) then
                           livecrootn_storage_to_xfer(p) = min(livecrootn(p), livecrootn_storage(p))*bgtr(p)
                           deadcrootn_storage_to_xfer(p) = min(deadcrootn(p), deadcrootn_storage(p))*bgtr(p)
                        end if

                        ! set phosphorus fluxes for shifting storage pools to transfer pools
                        frootp_storage_to_xfer(p) = min(frootp(p), frootp_storage(p))*bgtr(p)
                        if (woody(ivt(p)) == 1.0_r8) then
                           livecrootp_storage_to_xfer(p) = min(livecrootp(p), livecrootp_storage(p))*bgtr(p)
                           deadcrootp_storage_to_xfer(p) = min(deadcrootp(p), deadcrootp_storage(p))*bgtr(p)
                        end if
            ...
            if (season_decid(ivt(p)) == 1._r8) then
               ...
               if (offset_flag(p) == 1.0_r8) then
                  ...
                  if (offset_counter(p) == 0.0_r8) then
                     ! this code block wasoriginally handled by call cn_offset_cleanup(p)
                     ! inlined during vectorization
                     ...
                     ! reset the previous timestep litterfall flux memory
                     prev_leafc_to_litter(p) = 0._r8
                     if ((ivt(p) /= ndllf_dcd_brl_tree) .and. (ivt(p) /= nbrdlf_dcd_brl_shrub)) then
               ...
               if (dormant_flag(p) == 1.0_r8) then
                  ...
                  if ((ivt(p) == ndllf_dcd_brl_tree) .or. (ivt(p) == nbrdlf_dcd_brl_shrub)) then
                     if (ws_flag == 0._r8 .and. dayl(g) < off_pstart(ivt(p)) .and. t_ref2m(p) < off_tbase(ivt(p))) then
                        dayl_temp(p) = dayl_temp(p) + ((off_tbase(ivt(p)) - t_ref2m(p))**2*(dayl(g)/off_pstart(ivt(p))))*fracday
                     end if
                     if (ws_flag == 0._r8 .and. dayl_temp(p) > off_pend(ivt(p))) then
                        offset_flag(p) = 1._r8
                        dayl_temp(p) = 0._r8
                        offset_counter(p) = ndays_off(ivt(p))*secspday
                        prev_leafc_to_litter(p) = 0._r8
                     end if
                  else
                     if (ws_flag == 0._r8 .and. dayl(g) < crit_dayl(ivt(p))) then
                        offset_flag(p) = 1._r8
                        offset_counter(p) = ndays_off(ivt(p))*secspday
                        prev_leafc_to_litter(p) = 0._r8
                        prev_frootc_to_litter(p) = 0._r8
   ...
   subroutine CNStressDecidPhenology(num_soilp, filter_soilp, &
      ...
      associate ( &
         ...
         ndays_on => veg_vp%ndays_on, & ! Input:  [real(r8) (:) ] number of days for onset
         ndays_off => veg_vp%ndays_off, & ! Input:  [real(r8) (:) ] number of days for offset
            ...
            if (stress_decid(ivt(p)) == 1._r8) then
               ...
               if (dormant_flag(p) == 1._r8) then
                  ...
                  if (nu_com .ne. 'RD') then
                     crit_dayl(ivt(p)) = secspqtrday    !needed for BFB test
                  end if

                  ! only allow onset if dayl > 6hrs
                  if (onset_flag(p) == 1._r8 .and. dayl(g) <= crit_dayl(ivt(p))) then
                     onset_flag(p) = 0._r8
                  end if
                  ...
                  if (dayl(g) <= crit_dayl(ivt(p))) then
                     offset_flag(p) = 1._r8
                  end if
               ...
               if (onset_flag(p) == 1._r8) then
                  ...
                  ! set carbon fluxes for shifting storage pools to transfer pool
   ...
   subroutine CropPhenology(num_pcropp, filter_pcropp, &
      ...
      real(r8) ndays_on_crop ! number of days to fertilize
      ...
      ndays_on_crop = 20._r8 ! number of days to fertilize
            ...
            if (ivt(p) == nwcereal .or. ivt(p) == nwcerealirrig) then
               ...
               if (kmo .eq. plantmonth(p) .and. &
                  ...
                  if ((wf2(c) .ge. minwet .and. wf2(c) .le. maxwet) .and. &
                     ...
                     if (ivt(p) == nscereal .or. ivt(p) == nscerealirrig) then
                  ...
                  if (kmo .eq. (plantmonth(p) + 1) .and. gdd820(p) > 0._r8 .and. &
                        gdd820(p) /= spval) then
                     croplive(p) = .true.
                     cropplant(p) = .true.
                     idop(p) = jday
         ...
         if (croplive(p)) then
            ...
            if (leafout(p) >= huileaf(p) .and. hui(p) < huigrain(p) .and. idpp < mxmat(ivt(p))) then
               if (abs(onset_counter(p)) > 1.e-6_r8) then
                  onset_flag(p) = 1._r8
                  onset_counter(p) = dt
                  fert_counter(p) = ndays_on_crop*secspday
   ...
   subroutine CropPlantDate(num_soilp, filter_soilp, num_pcropp, filter_pcropp, &
      ...
      associate ( &
         ...
         do fp = 1, num_pcropp
            ...
            call calculate_eto(t_ref2m(p), netrad(p), eflx_soil_grnd(p), &
                                 forc_pbot(t), forc_rh(t), forc_wind(t), es, dt, ETout)
            ! monthly ETo
   ...
   subroutine CNOnsetGrowth(num_soilp, filter_soilp, &
      ...
      !
      ! !USES:
      use pftvarcon, only: ndllf_evr_brl_tree, ndllf_dcd_brl_tree, nbrdlf_dcd_brl_shrub
      use clm_varcon, only: secspday
      !
      ! !ARGUMENTS:
      integer, intent(in)    :: num_soilp       ! number of soil patches in filter
      ...
      integer :: p            ! indices
      integer :: fp           ! lake filter pft index
      real(r8):: t1, t2       ! temporary variable
      !-----------------------------------------------------------------------
      ...
      associate ( &
         ...
         onset_flag_root => cnstate_vars%onset_flag_root_patch, & ! Input:  [real(r8)  (:) ]  onset flag
         onset_counter_root => cnstate_vars%onset_counter_root_patch, & ! Input:  [real(r8)  (:) ]  onset days counter
         ...
         do fp = 1, num_soilp
            ...
            if (onset_flag(p) == 1._r8) then
               ...
               ! Root growth for the boreal PFTs are handled separately
               if ((ivt(p) /= ndllf_evr_brl_tree) .and. ((ivt(p)) /= ndllf_dcd_brl_tree) &
                  ...
                  if (woody(ivt(p)) == 1.0_r8) then
                     livecrootc_xfer_to_livecrootc(p) = t2*livecrootc_xfer(p)
                     deadcrootc_xfer_to_deadcrootc(p) = t2*deadcrootc_xfer(p)
                     livecrootn_xfer_to_livecrootn(p) = t2*livecrootn_xfer(p)
                     deadcrootn_xfer_to_deadcrootn(p) = t2*deadcrootn_xfer(p)
                     livecrootp_xfer_to_livecrootp(p) = t2*livecrootp_xfer(p)
                     deadcrootp_xfer_to_deadcrootp(p) = t2*deadcrootp_xfer(p)
                  end if
               end if
            end if

            if ((ivt(p) == ndllf_evr_brl_tree) .or. ((ivt(p)) == ndllf_dcd_brl_tree) &
                  .or. ((ivt(p)) == nbrdlf_dcd_brl_shrub)) then
               if (onset_flag_root(p) == 1._r8) then
                  if (onset_counter_root(p) <= dt) then
                     t2 = 1.0_r8/dt
                  else
   ...
   subroutine CNOffsetLitterfall(num_soilp, filter_soilp, &
      ...
      !
      ! !USES:
      use pftvarcon, only: npcropmin, grperc, grpnow, ndllf_evr_brl_tree, ndllf_dcd_brl_tree, nbrdlf_dcd_brl_shrub
      !
      ! !ARGUMENTS:
      integer, intent(in)    :: num_soilp       ! number of soil patches in filter
      integer, intent(in)    :: filter_soilp(:) ! filter for soil patches
      ...
      integer :: p, c               ! indices
      integer :: fp                 ! lake filter pft index
      real(r8):: t1, g1, g2, frac   ! temporary variable
      !-----------------------------------------------------------------------
      ...
      associate ( &
         ...
         do fp = 1, num_soilp
            ...
            if (ivt(p) == ndllf_evr_brl_tree) then
               ! Send newly formed leafc into storage for boreal evergreen needleleaf
               ! Gennaretti et al. 2017 Biogeosciences, 14, 4851-4866.
               !
            ...
            else if ((ivt(p) == ndllf_dcd_brl_tree) .or. (ivt(p) == nbrdlf_dcd_brl_shrub)) then
               ! Leaf litterfall only

               if (offset_flag(p) == 1._r8) then
                  ! Calculate the litterfall
                  if (offset_counter(p) == dt) then
                     t1 = 1.0_r8/dt
                  ...
                  ! calculate the leaf N&P litterfall and retranslocation
                  if (nu_com .eq. 'RD') then
                     ...
                     if (offset_counter(p) == dt) then
                        ...
                        leafn_to_litter(p) = max(min(leafc_to_litter(p)/max(leafc(p), 1.e-20_r8)*leafn(p), &
                                                      t1*leafn(p)), 0._r8)*0.38_r8
                        leafn_to_retransn(p) = max(min(leafc_to_litter(p)/max(leafc(p), 1.e-20_r8)*leafn(p), &
                                                         t1*leafn(p)), 0._r8)*0.62_r8

                        leafp_to_litter(p) = max(min(leafc_to_litter(p)/max(leafc(p), 1.e-20_r8)*leafp(p), &
                                                      t1*leafp(p)), 0._r8)*0.35_r8
                        leafp_to_retransp(p) = max(min(leafc_to_litter(p)/max(leafc(p), 1.e-20_r8)*leafp(p), &
                                                         t1*leafp(p)), 0._r8)*0.65_r8
                     end if
                  end if
               ...
               if (offset_flag(p) == 1._r8) then
                  ...
                  if (nu_com .eq. 'RD') then
                     ...
                     if (offset_counter(p) == dt) then
                        ...
                        leafn_to_retransn(p) = max(min(leafc_to_litter(p)/max(leafc(p), 1.e-20_r8)*leafn(p), &
                                                         t1*leafn(p)), 0._r8)*0.62_r8
                        frootn_to_litter(p) = max(min(frootc_to_litter(p)/max(frootc(p), 1.e-20_r8)*frootn(p), &
                                                      t1*frootn(p)), 0._r8)
                        ...
                        leafp_to_retransp(p) = max(min(leafc_to_litter(p)/max(leafc(p), 1.e-20_r8)*leafp(p), &
                                                         t1*leafp(p)), 0._r8)*0.65_r8
                        frootp_to_litter(p) = max(min(frootc_to_litter(p)/max(frootc(p), 1.e-20_r8)*frootp(p), &
                                                      t1*frootp(p)), 0._r8)
                     end if
                  end if
